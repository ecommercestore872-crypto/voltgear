...
export async function POST(request: Request) {
  ...
  if (!body || typeof body !== "object") {
    return NextResponse.json(
      { error: "Send ad spend for this date range." },
      { status: 400 },
    );
  }
  const raw = body as Record<string, unknown>;
  const now = new Date();
  const preset = parseAnalyticsPreset(
    typeof raw.preset === "string" ? raw.preset : "last30",
  );
  ...
  const bySource =
    raw.bySource &&
    typeof raw.bySource === "object" &&
    !Array.isArray(raw.bySource)
  ...
  return NextResponse.json({
    ok: true,
    range,
    spend: spendForRange(next, range),
  });
}
