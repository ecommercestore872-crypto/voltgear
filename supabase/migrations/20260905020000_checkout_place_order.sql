-- Place an order and decrement only products/variants that have quantity set.
-- NULL quantity = unlimited (do not block launch for unconfigured catalog).
...
BEGIN
    ...
    FOR item IN SELECT * FROM jsonb_array_elements(p_items) ORDER BY value->>'slug', value->>'variantKey' LOOP
        ...
        IF item.value->>'variantKey' IS NOT NULL AND item.value->>'variantKey' <> '' THEN
            SELECT pv.id, pv.quantity INTO v_variant_id, v_current_stock
            FROM products p
