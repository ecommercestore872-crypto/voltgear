...
import {
  SELFIESTICKS_DATA,
  SELFIESTICKS_KEEP_SLUGS,
} from "./data/selfiesticks-catalog";
...
const KEEP = new Set<string>(SELFIESTICKS_KEEP_SLUGS);

async function seed() {
  console.log(`Upserting ${SELFIESTICKS_DATA.length} selfie stick products...`);
  ...
  for (const prod of SELFIESTICKS_DATA) {
    ...
    if (prod.productVideoUrl) {
      row.product_video = { url: prod.productVideoUrl };
    }
  ...
  const { data: extras, error: extrasErr } = await supabase
    .from("products")
    .select("id, slug, name")
    .eq("category", "selfie-stick");

  if (extrasErr) {
    console.error("Failed to list selfie-stick products for cleanup:", extrasErr);
  } else {
    for (const row of extras ?? []) {
      ...
      console.log(
        error
          ...
          : `Unpublished non-catalog selfie-stick: ${row.slug}`
      );
    }
  }
  ...
  if (liveErr) {
    ...
    console.log(`\nPublished selfie-stick products (${live?.length ?? 0}):`);
    for (const p of live ?? []) {
