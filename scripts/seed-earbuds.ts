...
const KEEP = new Set<string>(EARBUDS_KEEP_SLUGS);

async function seed() {
  console.log(`Upserting ${EARBUDS_DATA.length} earbud products...`);
  ...
  if (extrasErr) {
    console.error("Failed to list earbud products for cleanup:", extrasErr);
  } else {
    for (const row of extras ?? []) {
      ...
      console.log(
        error
          ...
          : `Unpublished non-catalog earbud: ${row.slug}`
      );
    }
  }
  ...
  console.log(`\nPublished earbud products (${live?.length ?? 0}):`);
  for (const p of live ?? []) {
