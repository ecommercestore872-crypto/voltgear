...
import {
  SMARTWATCHES_DATA,
  SMARTWATCHES_KEEP_SLUGS,
} from "./data/smartwatches-catalog";
...
const KEEP = new Set<string>(SMARTWATCHES_KEEP_SLUGS);

async function seed() {
  console.log(`Upserting ${SMARTWATCHES_DATA.length} smartwatch products...`);
  ...
  if (extrasErr) {
    console.error("Failed to list smartwatch products for cleanup:", extrasErr);
  } else {
    for (const row of extras ?? []) {
      ...
      console.log(
        error
          ...
          : `Unpublished non-catalog smartwatch: ${row.slug}`
      );
    }
  }
  ...
  console.log(`\nPublished smartwatch products (${live?.length ?? 0}):`);
  for (const p of live ?? []) {
    console.log(`- ${p.slug} | ${p.badge} | ${p.name}`);
  }

  console.log("Smartwatches seeding process completed!");
}
